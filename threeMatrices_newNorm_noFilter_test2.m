%% calculate dise`ZZase-target ranking using wenhui's normalization method
%function [] = threeMatrices_newNorm()
function [transMat12,transMat23] = threeMatrices_newNorm_noFilter_test2(transMat12, simMat1, simMat2, simMat3, transMat23,decay,min_sim)
...
fprintf('\n');
    ...
    while i< 50 && (error1>thr || error2>thr)
    i=i+1;
%	fprintf('iteration:%d %f %f\n',i,error1,error2);%(error1>thr || erro2>thr));
        %fprintf('iteration %d\n', i);
        error1 = max(max(abs(DR2 - DR1)));
